...
class IssueDetector:
    ...
    def detect_duplication_issues(self, all_results, similarity_threshold=0.85):
        """
        Detect content duplication across all crawled pages using MinHash + LSH.
        Scales to large crawls without O(n²) pairwise comparisons.
        """
        issues = []
        ...
        for result in all_results:
            ...
            mh = self._build_minhash(result)
            # LSH keys must be strings and unique
            key = url
            try:
                lsh.insert(key, mh)
                minhashes[key] = mh
        ...
        # Query each page for near-duplicates
        reported = set()
        for url, mh in minhashes.items():
            neighbors = lsh.query(mh)
            for neighbor in neighbors:
                if neighbor == url:
                    continue
                pair = tuple(sorted([url, neighbor]))
                if pair in reported:
                    continue
                reported.add(pair)
                issues.append({
                    ...
                    'details': f'Content is similar to {neighbor}'
                })
                issues.append({
                    'url': neighbor,
                    'type': 'warning',
                    'category': 'Duplication',
                    'issue': 'Duplicate Content Detected',
                    'details': f'Content is similar to {url}'
                })

        with self.issues_lock:
            self.detected_issues.extend(issues)

    def _build_minhash(self, result):
        """Build a MinHash signature from a page's SEO fields."""
        text = ' '.join(filter(None, [
            result.get('title', '').lower().strip(),
            result.get('meta_description', '').lower().strip(),
            result.get('h1', '').lower().strip(),
        ]))
        mh = MinHash(num_perm=128)
        # Shingle into 3-word ngrams
        words = text.split()
        shingles = [' '.join(words[i:i+3]) for i in range(len(words) - 2)] if len(words) >= 3 else words
